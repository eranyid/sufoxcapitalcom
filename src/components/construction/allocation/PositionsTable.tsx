...
export function PositionsTable({ positions, onUpdate, onDelete }: PositionsTableProps) {
  ...
  return (
    <Card className="overflow-hidden">
      {/* Desktop Header */}
      <div className="hidden sm:grid grid-cols-12 gap-2 px-4 py-3 bg-muted/30 border-b border-border/50 text-xs">
      ...
      <div className="divide-y divide-border/30">
        {sortedPositions.map((position, index) => (
          <div key={position.id} className={cn(
              ...
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <Badge variant="outline" className={cn("text-[9px] font-medium border", ASSET_TYPE_STYLES[position.assetType] || 'bg-muted/50 text-muted-foreground')}>
                      {ASSET_TYPE_LABELS[position.assetType]}
                    </Badge>
                    <span className="text-[10px] text-muted-foreground">{REGION_LABELS[position.region]} • {position.currency}</span>
                  </div>
                </div>
                <div className="flex items-center gap-1">
                  ...
                  <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-destructive" onClick={() => onDelete(position.id)}>
                    <Trash2 size={14} />
                  </Button>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Button size="icon" variant="outline" className="h-7 w-7 rounded-full border-border/50 shrink-0" onClick={() => incrementAllocation(position, -1)} disabled={position.allocation <= 0}>
                  <Minus size={12} />
                </Button>
                <Slider
                  ...
                  className="flex-1"
                />
                <Button size="icon" variant="outline" className="h-7 w-7 rounded-full border-border/50 shrink-0" onClick={() => incrementAllocation(position, 1)} disabled={position.allocation >= 100}>
                  <Plus size={12} />
                </Button>
              </div>
            </div>
            ...
            {/* Desktop Layout */}
            <div className={cn(
              "hidden sm:grid grid-cols-12 gap-2 px-4 py-3.5 items-center",
              "hover:bg-muted/20"
            )}>
              <div className="col-span-3">
                <div className="flex items-center gap-2">
              ...
              <div className="col-span-4">
                <div className="flex items-center gap-2">
                  <Button size="icon" variant="outline" className={cn("h-7 w-7 rounded-full border-border/50 transition-all", "hover:bg-destructive/10 hover:border-destructive/50 hover:text-destructive", "active:scale-95")} onClick={() => incrementAllocation(position, -1)} disabled={position.allocation <= 0}>
                    <Minus size={12} />
                  </Button>
                  <div className="flex-1 flex items-center gap-2">
                    <Slider value={[position.allocation]} onValueChange={([value]) => handleAllocationChange(position.id, value)} onValueCommit={() => setActiveSlider(null)} onPointerDown={() => setActiveSlider(position.id)} max={50} min={0} step={0.5} className="flex-1" />
                    <span className={cn("font-mono text-sm font-bold w-12 text-right tabular-nums transition-colors", position.allocation >= 20 ? "text-amber-400" : position.allocation >= 10 ? "text-foreground" : "text-muted-foreground")}>
                      {position.allocation.toFixed(1)}%
                    </span>
                  </div>
                  <Button size="icon" variant="outline" className={cn("h-7 w-7 rounded-full border-border/50 transition-all", "hover:bg-emerald-500/10 hover:border-emerald-500/50 hover:text-emerald-500", "active:scale-95")} onClick={() => incrementAllocation(position, 1)} disabled={position.allocation >= 100}>
                    <Plus size={12} />
                  </Button>
                </div>
              </div>
              <div className="col-span-2">
                <Badge variant="outline" className={cn("text-[10px] font-medium border", ASSET_TYPE_STYLES[position.assetType] || 'bg-muted/50 text-muted-foreground')}>
                  {ASSET_TYPE_LABELS[position.assetType]}
                </Badge>
              </div>
              <div className="col-span-2">
              ...
              <div className="col-span-1 flex justify-end">
                <Button size="icon" variant="ghost" className="h-7 w-7 opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-destructive hover:bg-destructive/10" onClick={() => onDelete(position.id)}>
                  <Trash2 size={14} />
                </Button>
              </div>
            </div>
      ...
      {/* Footer Summary */}
      <div className="px-3 sm:px-4 py-2.5 bg-muted/20 border-t border-border/50 flex items-center justify-between">
        <span className="text-xs text-muted-foreground">
          {positions.length} position{positions.length !== 1 ? 's' : ''} 
        </span>
        <div className="flex items-center gap-2 sm:gap-3">
          <span className="hidden sm:inline text-[10px] text-muted-foreground">Drag slider or use +/- buttons</span>
